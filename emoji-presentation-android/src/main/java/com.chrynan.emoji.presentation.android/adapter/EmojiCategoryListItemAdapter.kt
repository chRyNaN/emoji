...
@Adapter
class EmojiCategoryListItemAdapter(private val listener: EmojiCategoryListItemSelectedListener) : AnotherAdapter<EmojiCategoryListItemViewModel>() {
    ...
    override fun onCreateView(parent: ViewGroup, inflater: LayoutInflater, viewType: ViewType): View =
        inflater.inflate(com.chrynan.emoji.presentation.android.R.layout.adapter_emoji_category_list_item, parent, false)

    override fun View.onBindItem(item: EmojiCategoryListItemViewModel, position: Int) {
        val adapterEmojiCategoryTitleTextView = findViewById<TextView>(com.chrynan.emoji.presentation.android.R.id.adapterEmojiCategoryTitleTextView)
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
            adapterEmojiCategoryTitleTextView?.tooltipText = if (item.showEmojiAsTitle) null else item.category
        }
        adapterEmojiCategoryTitleTextView?.isActivated = item.isSelected
        adapterEmojiCategoryTitleTextView?.setOnClickListener { listener.onEmojiCategoryListItemSelected(item) }
    }
}
